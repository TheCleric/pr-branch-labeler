...
describe('Config entry', () => {
  ...
  describe('Dynamic headRegExp and baseRegExp usage', () => {
    ...
    it("adds the 'sprint10' label for 'bugfix/FOO-42-changes' to 'sprint/10'", async () => {
      // Arrange
      const entry = new ConfigEntry({ label: 'sprint$1', baseRegExp: /sprint[/](.*)/ });

      // Act
      const label = entry.getLabel('bugfix/FOO-42-changes', 'sprint/10');

      // Assert
      expect(label).toEqual('sprint10');
      expect.assertions(1);
    });
  ...
  describe('Disallows mixing of matching and regex for same branch', () => {
    it('errors when using head and headRegExp', async () => {
      // Assert
      expect(() => {
        new ConfigEntry({ label: 'bugfix', head: 'bugfix/*', headRegExp: /bugfix[/].*/ });
